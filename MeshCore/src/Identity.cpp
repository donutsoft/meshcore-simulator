...
#include "../simulator/SimulatorIdentityRegistry.h"
#endif
...
#ifdef MESHCORE_SIMULATOR
  // Simulator repeaters use generated Ed25519 keypairs whose public-key prefix
  // is replaced with an observed database prefix. Only those explicitly
  // registered identities bypass signature verification; companion identities
  // continue through the normal Ed25519 implementation below.
  if (simulator::isForcedRepeaterIdentity(pub_key)) return true;
#endif
#ifdef USE_CC310_HW_CRYPTO
